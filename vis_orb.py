...
###############################
all_trajs = []
dir_path = '/home/ao/Projects/ORB_SLAM2/mono_mh02/'
index = 0
for txt_name in sorted(os.listdir(dir_path)):
    ...
    ax.plot(xs, ys, linewidth=2, color=colors[index], label= 'Traj. ' + str(index+1), path_effects=[pe.Stroke(linewidth=5, foreground='darkslategray'), pe.Normal()])
    index = index + 1
...
#for traj in all_trajs:
    ...
    errors = traj_translation_error(test_traj, ground_truth_traj)
    print('Absolute')
    error_log(errors)
    rela_errors = traj_relative_translation_error(test_traj, ground_truth_traj)
    print('Relative:')
    error_log(rela_errors)
    print('-------------------')

#ax.set_xlim([9.0, 13.0])
#ax.set_ylim([-3.75, -1.75])
...
plt.savefig('orb_mono_mh02.eps', format='eps')
plt.show()
exit(0)
